...
export default class SourceController {
  ...
  constructor(media: MediaPlayerElement, streamData: Anima.RAW.EpisodeStream) {
    ...
    //----------------------------------------//
    // START MANIPULATING SOURCE & SUBTITLE
    //----------------------------------------//
    this._mediaPlayer.addEventListener('can-load', (ev) => {
      // console.log("Initializing sourceController after loading metadata")
      this.initialize()
    }, { once: true })
  }
      ...
      if (opiniatedStream) {
        ...
        .then( newStreamData => {
          ...
          this._mediaPlayer.addEventListener('can-play', () => {
            writeAtom(playerSwitchingStream, false)
  ...
  public getOpiniatedStream() {
    ...
    if (userPrefered === '') return Object.keys(availableAudios).map( l => {
      const s = availableAudios[l]
    ...
    return Object.keys(availableAudios).map(locale => {
      if (locale === userPrefered) {
        return availableAudios[locale]
      }
    })[0]
  }
