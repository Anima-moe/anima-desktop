...
export default memo(function Captions() {
  ...
  const { data, error, isLoading } = useQuery(
    `subtitles/${streamConfig.subtitleURL}.${streamConfig.streamLocale}`,
    () => {
      return axios.get(streamConfig.subtitleURL).then((g) => g.data)
    }
  )
  ...
  const textLine = useMemo(() => {
    ...
    const currentCaption = captions.find((caption) =>
      shouldDisplayCaption(caption.data, currentTime)
    )
  ...
  return (
    <div className="absolute bottom-24 z-[50] flex w-full flex-col items-center justify-end text-3xl transition-all duration-300 media-user-idle:bottom-10">
      {/* @ts-ignore-error |  */}
