...
export default class SourceController {
  ...
  private initialize() {
    ...
    if (opiniatedStream) {
      Stream.get(this._episodeID, opiniatedStream.external_id).then((newStreamData) => {
        if (
          (newStreamData.data.mp4 || newStreamData.data.hls) ===
          readAtom(playerStreamConfig).streamURL
        )
          return
        ...
        writeAtom(playerStreamConfig, {
          ...
          streamURL: `http://localhost:15411/${btoa(
            newStreamData.data?.mp4?.filter(
              (s) => s.height === this.getOpiniatedQuality()?.height
            )?.[0].src || newStreamData.data.hls
          )}.${streamExtension}`,
          streamLocale: this.getLocaleFromStreamExternalID(
            this._streamData,
            opiniatedStream.external_id
          ),
        })
      ...
      writeAtom(playerStreamConfig, {
        ...readAtom(playerStreamConfig),
        streamURL: `http://localhost:15411/${btoa(
          this._streamData?.mp4?.filter((s) => s.height === this.getOpiniatedQuality()?.height)?.[0]
            .src || this._streamData.hls
        )}.${streamExtension}`,
        streamLocale: 'ja-JP',
      })
  ...
  public getOpiniatedStream() {
    ...
    if (userPrefered === '')
      return Object.keys(availableAudios).map((locale) => {
  ...
  public async requestSourceChange(newSource: { original: boolean; external_id: string }) {
    ...
    if (
      (newStreamData.data.mp4 || newStreamData.data.hls) === readAtom(playerStreamConfig).streamURL
    )
      return
    const streamExtension = newStreamData.data.mp4 ? 'mp4' : 'm3u8'
    ...
    writeAtom(playerStreamConfig, {
      ...readAtom(playerStreamConfig),
      streamURL: `http://localhost:15411/${btoa(
        newStreamData.data?.mp4?.filter((s) => s.height === this.getOpiniatedQuality()?.height)?.[0]
          .src || newStreamData.data.hls
      )}.${streamExtension}`,
      streamLocale: locale,
    })
